...
######################################################################################
#
# Description: simulation of rough Bergomi model
#
######################################################################################
######################################################################################

# Original implementation by Ryan McCrickerd. 
# Based on the hybrid simulation scheme of Bennedsen, Lunde, and Pakkanen (2017). 
# See: https://github.com/ryanmccrickerd/rough_bergomi 

# Modifications:
# - function Y has a special case for a==0, to directly use standard Brownian motion

import numpy as np
from thesis_code.models.rough_bergomi.utils import *
...
class rBergomi(object):
    ...
    def Y(self, dW):
        ...
        if self.a == 0:
            ...
            Y[:, 1:1 + self.s] = np.cumsum(dW[:, :, 0], axis=1)
            return Y
        else:
